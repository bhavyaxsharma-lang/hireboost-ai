...
// POST /resume/rewrite — AI rewrites the full resume, gated behind a verified payment
router.post("/rewrite", async (req, res) => {
  const { resumeText, atsScore, missingKeywords, suggestions, strengths, overallFeedback, jobTitle, paymentId } =
    req.body as {
      resumeText?: string;
      atsScore?: number;
      missingKeywords?: string[];
  ...
  if (!resumeText || !paymentId) {
    res.status(400).json({ error: "Missing resumeText or paymentId." });
    return;
  }

  // Verify payment token exists in session
  const verifiedPayments: string[] = req.session.verifiedPayments ?? [];
  if (!verifiedPayments.includes(paymentId)) {
    res.status(403).json({ error: "Payment not verified for this session." });
    return;
  }

  // One-time use — remove the payment token after use
  req.session.verifiedPayments = verifiedPayments.filter((id) => id !== paymentId);

  const prompt = `You are an expert resume writer and career coach. Your task is to rewrite and significantly improve the following resume.
...
INSTRUCTIONS:
1. Rewrite the entire resume to address ALL the missing keywords and apply ALL the improvement suggestions.
2. Preserve the person's actual experience, education, and achievements — do not invent new ones.
3. Use strong action verbs, quantify achievements where possible, and write in a professional ATS-friendly format.
4. Structure the resume clearly with sections: Summary, Experience, Skills, Education (and other relevant sections if present).
5. Naturally incorporate the missing keywords throughout the resume.
6. Make the language impactful, concise, and tailored${jobTitle ? ` for a ${jobTitle} role` : ""}.
7. Return ONLY the improved resume text — no commentary, no markdown headers like "Improved Resume:", just the resume content.`;
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert resume writer. Rewrite resumes to be ATS-optimized and compelling.",
        },
